...
public class IndexService {
    ...
    @PostConstruct
    // when starting program, we don\t want to recreate index since that would delete all data in index.
    // so only manual explicitly set flag will recreate index
    ...
    public void recreateIndices(final boolean deleteExisting) {
        ...
        for (final String indexName : INDICES) {
            try {
                ...
                final String mappings = loadMappings(indexName); // mappings and index should have same name
                if (mappings != null) {
